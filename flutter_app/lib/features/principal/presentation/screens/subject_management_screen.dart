...
class _SubjectManagementScreenState extends ConsumerState<SubjectManagementScreen> {
    ...
    return Scaffold(
      ...
      body: subjectsAsync.when(
        data: (subjects) => subjects.isEmpty
            ...
            : RefreshIndicator(
                onRefresh: () async => ref.invalidate(subjectsProvider(widget.classId)),
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _SubjectCard(
                      ...
                      onAssignTeacher: () => _showAssignTeacherDialog(subject, teachersAsync),
                    );
                  },
                ),
              ),
              ...
              builder: (context, ref, _) {
                ...
                return LoadingButton(
                  onPressed: () async {
                    ...
                    final success = await ref.read(subjectManagementProvider.notifier).createSubject(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            CustomTextField(controller: descController, label: 'Description', maxLines: 2),
          ],
        ),
        actions: [
          ...
          Consumer(
            builder: (context, ref, _) {
              ...
              return LoadingButton(
                onPressed: () async {
                  final success = await ref.read(subjectManagementProvider.notifier).updateSubject(
                    subject.id,
                    name: nameController.text.trim(),
  ...
  void _showAssignTeacherDialog(SubjectModel subject, AsyncValue<List<TeacherModel>> teachersAsync) {
          ...
          actions: [
            ...
            Consumer(
              builder: (context, ref, _) {
                return TextButton(
                  onPressed: () async {
                    await ref.read(subjectManagementProvider.notifier).updateSubject(
                      subject.id,
  ...
  Future<void> _confirmDelete(SubjectModel subject) async {
    ...
    if (confirmed == true) {
      await ref.read(subjectManagementProvider.notifier).deleteSubject(subject.id);
      ref.invalidate(subjectsProvider(widget.classId));
    }
  }
}
    ...
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            if (subject.description != null) Text(subject.description!, maxLines: 1, overflow: TextOverflow.ellipsis),
            Text(
              subject.teacherName ?? 'No teacher assigned',
              style: TextStyle(color: subject.teacherName != null ? AppColors.success : AppColors.textSecondary),
            ),
          ],
        ),
        trailing: PopupMenuButton(
          itemBuilder: (context) => [
            const PopupMenuItem(value: 'teacher', child: Text('Assign Teacher')),
            const PopupMenuItem(value: 'edit', child: Text('Edit')),
            const PopupMenuItem(value: 'delete', child: Text('Delete', style: TextStyle(color: AppColors.error))),
          ],
          onSelected: (value) {
            switch (value) {
              case 'teacher': onAssignTeacher(); break;
